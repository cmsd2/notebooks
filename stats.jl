...
# ╔═╡ 440ce5c8-e8b7-11f0-b787-318b41b6b7a7
begin
	using Distributions
	using StatsBase
	using CSV
...
# ╔═╡ be12166f-36c4-4c41-8843-94f0847368ca
pyplot()

# ╔═╡ 96440a28-3b4c-4dd8-a836-f5399b9e6db1
begin
...
# ╔═╡ db9c3c9c-df70-4f38-84f9-e7d1654122af
StatsBase.describe(age)

# ╔═╡ ce19eb82-207e-4e87-9a07-8a7c9d1a81c2
StatsBase.summarystats(wcc)

# ╔═╡ 71e7e6a1-0745-4849-962f-0cb22e9ab946
data = DataFrame(Age = age, WCC = wcc, CRP = crp, Treatment = treatment, Result = result)

# ╔═╡ 0f7379e5-5957-43dc-91bf-9cd0abae36e9
begin
	dataA = data[data[!, :Treatment] .== "A", :];
	dataB = data[data[!, :Treatment] .== "B", :];
	size(dataA), size(dataB)
end

# ╔═╡ badb04b5-443a-4326-a0e8-988a1f007fce
describe(data)

# ╔═╡ d6e1501a-5599-4384-bd7f-936164c75187
combine(groupby(data, :Treatment), :Treatment => size)

# ╔═╡ 74921c9a-e949-4d4f-933a-d1a6684bc7dd
combine(groupby(data, :Treatment), :Age => mean)

# ╔═╡ 41c342bb-c4dc-4733-8381-f2afdd09c919
@chain data begin
	@df density(:Age, group = :Treatment, title = "ages by treatment group", xlab = "Age", ylab = "Distribution", legent = :topright)
end

# ╔═╡ ee6e1bdb-b0bf-425c-8ddf-a94a471fb748
@df data StatsPlots.boxplot(:Treatment, :WCC, lab = "WCC", title = "White cell count by treatment group", xlab = "Groups", ylab = "WCC")

# ╔═╡ 09095579-c8bd-4ec0-bfea-3f1c51491d0c
HypothesisTests.EqualVarianceTTest(dataA[!, :Age], dataB[!, :Age])

# ╔═╡ b72d11b2-3c61-47a6-8350-3b023c3b095b
pvalue(EqualVarianceTTest(dataA.WCC, dataB.WCC))

# ╔═╡ 0fb22917-89c9-42f0-88a9-61754f5e1196
UnequalVarianceTTest(dataA.CRP, dataB.CRP)

# ╔═╡ a4603d95-5f6a-45ec-983b-1bea64926eef
fit(LinearModel, @formula(CRP ~ 1), data)

# ╔═╡ 21b4010a-7ccf-45ad-be6c-e58887c231d0
fit(LinearModel, @formula(CRP ~ Age), data)

# ╔═╡ 51e5de7f-8eb1-4255-ba85-25f113560b2b
...
manifest_format = "2.0"
project_hash = "99abfff1de85b091f05805dfdc322f6363dc895f"
